    ...
    try {
        ...
        for (let i = 0; i < numDraws; i++) {
            ...
            if (drawnItem && drawnItem.productName) {
                ...
                display = `<div class="item-container" style="margin-left:320px;">${drawnItem.productImage}</div>`;
            } else {
                i--; // 減少i以重新執行本次抽獎
            }
        }
        ...
        if (maxResult) {
            showGachaResult(maxResult.scaledProbability, allImages);
...
function showGachaResult(scaledProbability, allImages) {
    // 將四個button關掉顯示
